export function generateCssSelector(el) {
  ...
  while (el.nodeType === Node.ELEMENT_NODE) {
    ...
    if (sibling) {
      var hasSibling =
        el.previousElementSibling.tagName == elementType ? true : false;
    }
    while (sibling) {
...
export function extract_data_list(dictOfContet) {
  let data = {};
  for (i in dictOfContet.keys()) {
    let element = document.querySelectorAll(dictOfContet[i]);
    data = { ...data, [i]: extract_data(element) };
  }
  return data;
}
